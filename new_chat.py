...
def handle_food_info_get(food_name, user_info):
    ...
    # # EER
    age = float(user_age)
    age_range1 = get_eer_age_range(age)
    PAL_MJ, PAL_kcal = pal_info(age_range1, user_gender, user_PA)
    eer_info_list = {
        "PAL(MJ)": PAL_MJ,
        "PAL(kcal)": PAL_kcal
    }
    ...
    # # PROTEIN
    age_range2 = get_protein_fat_age_range(age)
    EAR, RNI = ear_rni_info(age_range2, user_gender)
    protein_info_list = {
        "EAR": EAR,
        "RNI": RNI
    }
    ...
    # # FAT_CARB
    age_range3 = get_protein_fat_age_range(age)
    fat_carb_info_list = carbohydrates_info(age_range3)
    user_desc=gen_user_desc(user_info)
    
    content = ask_llm(food_name, filtered_df1, filtered_df2, filtered_df3, user_desc)
    import markdown2
    if isinstance(content, str): 
        content = markdown2.markdown(content, extras=["strip-markdown"])
